...
| 260611-ent | Peer-review batch 4/4: Sidebar.tsx 823→631 (drag/drop → `useSidebarDragDrop.ts`, Shift+F10 menu → `SidebarResetMenu.tsx`, pure refactor) + base64/hash/unix-time/uuid-ulid inline toggles → shared SegmentedControl (-106 lines, aria byte-identical). e2e 15/15 + vitest 816/816 + tsc/eslint green, tests/specs UNMODIFIED, decoder untouched. **⏸ human drag walkthrough pending** (see Pending Todos) | 2026-06-11 | c941dda0 + c6b2bf9a | [260611-ent-sidebar-tsx-decomposition-drag-drop-rese](./quick/260611-ent-sidebar-tsx-decomposition-drag-drop-rese/) |
...
- **Quick 260611-ent human-verify pending (tonight 2026-06-11):** manual drag walkthrough on `pnpm tauri dev` — pinned-group reorder, unpinned end-zone drop, no cross-boundary indicator, persistence after reload + glance at 4 migrated toggles (base64/hash/unix-time/uuid-ulid). Code landed (c941dda0 SegmentedControl ×4, c6b2bf9a Sidebar 823→631 via `useSidebarDragDrop.ts` + `SidebarResetMenu.tsx`); all automated gates green (e2e 15/15, vitest 816/816, specs/tests unmodified). Drag has NO automated net (dragDropEnabled=false). Any regression → fix in `src/components/useSidebarDragDrop.ts` only, re-run gate.
