...
- `tauri.conf.json` already sets `app.windows[0].visible: false` [VERIFIED: tauri.conf.json:21]. The lib.rs comment claims `visible:false` *"avoids the restore-flash"*, but that is INCOMPLETE — see Open Questions (RESOLVED) #1. **CORRECTION (verified): the window is NOT shown by the webview today. The real normal-launch reveal is the window-state plugin's NATIVE `restore_state` `self.show()`** (it fires on window-ready, BEFORE the webview prefs load, because `StateFlags::default() == all()` includes `VISIBLE`). So there IS a native reveal to neutralize for the no-flash start-in-tray path — a pure webview gate is insufficient. Plan 02 drops `StateFlags::VISIBLE` so the plugin restores geometry but no longer auto-shows, making the webview `revealOnStartup()` the sole reveal.
...
1. **Where does the window become visible on a normal launch today? — RESOLVED.**
   - **Answer (verified):** The reveal is the **window-state plugin's native `restore_state` `self.show()`**, NOT a webview show. There is NO `platform.window.show()` anywhere in `src/main.tsx` / `App.tsx` / router — the only webview `show()` is inside `summon.ts` (the user-triggered summon path). On a normal launch the window becomes visible at the NATIVE layer: `src-tauri/src/lib.rs` (~L33) registers `tauri_plugin_window_state::Builder::default().build()`; `StateFlags::default() == StateFlags::all()` INCLUDES `VISIBLE` (`const VISIBLE = 1 << 3`), and on window-ready the plugin's `restore_state` ends with `if flags.contains(VISIBLE) && should_show { self.show()?; self.set_focus()?; }` (`should_show` defaults to `true`). [VERIFIED against tauri-plugin-window-state-2.4.1/src/lib.rs: L56, L64-65, L179, L247-248, L262-264.] This native show fires BEFORE the webview prefs load, so a webview-only gate cannot prevent a start-in-tray flash.
   - **Implication for the gate (Assumption A2 corrected):** Assumption A2 was WRONG — the App-root does NOT call `show()`. The start-in-tray no-flash gate must attach at the NATIVE layer: drop `StateFlags::VISIBLE` from the window-state plugin so it restores geometry (SHL-05/D-11) but never auto-shows, leaving the webview `prefsLoaded`-gated `revealOnStartup()` (gated on `!startInTray`) as the sole reveal.
   - **Resolution lands in:** Plan 24-02 Task 2 — `.with_state_flags(StateFlags::all() ^ StateFlags::VISIBLE)` in `src-tauri/src/lib.rs` plus the webview `revealOnStartup()` in `src/main.tsx` / `src/shell/startupReveal.ts`. (Do NOT use `skip_initial_state("main")` — that would also skip geometry restore; dropping only VISIBLE is the surgical fix.) The single-instance and tray `unminimize→show→set_focus` calls (lib.rs L19-25 / L234-263) are user-triggered reveals and stay untouched.
