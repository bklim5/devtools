...
- **Current state: Phase 7 (Formatters) complete — v1.1 in progress** (2026-06-02, verified 5/5 must-haves, human-signed-off on a fresh `tauri build`; real-WKWebView e2e 10/10 + WCAG-AA PASS). The app now ships JSON + XML formatter tools (FMT-01..08) behind a shared `FormatterView`, registry-only, zero new runtime deps. **Next: Phase 8** (StatusBar size-readout cleanup, UIX-01 — make `byteCount` opt-in). v1.0 (Phases 1–6) shipped 2026-06-01: foundation/harness, shell, hero+encoding+UX, the catalogue tools, persistence/window-geometry, and a distributable self-updating macOS app (DST-01/DST-02). **Carry-forwards (not blockers):** FormatterView narrow-width vertical stacking (UX-05, polish); Gatekeeper-clean notarisation pending Apple Developer enrolment (D-02); NAT-01 (G-05-1) parked; 3 minor a11y polish follow-ups from the updater UI review.
...
---
*Last updated: 2026-06-02 after Phase 7 (Formatters) completion & sign-off — v1.1 in progress (Phase 8 next)*
