...
**Active: v1.4 "Reorderable Tools"** (started 2026-06-04) — a focused single-feature milestone (Phase 16): user-reorderable sidebar tool list (drag + accessible keyboard path), order persisted, promoted from backlog 999.6. The first milestone to add a *personalization* capability rather than a tool. **Phase 16 COMPLETE (2026-06-05)** — verification passed 11/11 must-haves, all 7 REORD requirements delivered (`toolOrder` overlay over the registry, handle-initiated native drag + Alt+↑/↓ keyboard reorder + `aria-live`, persisted, reconciled, reset-to-default), human-signed-off on the real WKWebView; full suite 668/668, decoder 19/19 untouched, zero new deps, gsd-ui-review WCAG-AA 22/24 (findings fixed). Milestone v1.4 (single phase) ready to close out. Remaining backlog parked (999.1 tool wishlist, 999.2 CI, 999.3 theme settings, 999.4 CLI, 999.5 Protobuf schema-file).
