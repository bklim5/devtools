...
**Progress: Milestone v1.2 COMPLETE (Phases 9–11, all verified).** Phase 9 (pure release core + housekeeping) **complete & verified** (2026-06-02, 5/5) — `src/lib/release/version.ts` + `manifest.ts`, fully unit-tested; `Cargo.toml` reconciled 0.1.0→0.2.1 (REL-02), stale `latest.json` untracked + gitignored (REL-08). Phase 10 (`bump-and-tag` driver) **complete & verified** (2026-06-02, 5/5) — thin `scripts/bump-and-tag.mjs` over the pure `bumpPlan.ts` core, wired to `pnpm release:bump`: lockstep 3-manifest bump + lockfile regen (REL-01/REL-03), annotated `vX.Y.Z` tag + push to private origin (REL-04), `--dry-run` (REL-10) + preflights (REL-11). **Live-proven:** real `v0.2.2` bump-commit-tag pushed to origin. Phase 11 (`build-and-publish` driver + universal binary + safety rails) **complete & verified** (2026-06-03, 5/5) — thin `scripts/build-and-publish.mjs` over the pure `publishPlan.ts` core, wired to `pnpm release:publish`: universal `tauri build` + `lipo` both-arch assert (REL-05), fresh-`.sig` dual-key `latest.json` (REL-06), cross-repo `gh` publish assets-first/manifest-last (REL-07), `APPLE_*` passthrough/ad-hoc default (REL-09), post-publish `curl` served-version verify (REL-12), `--dry-run` + build-time preflights. **Live-proven (the milestone's load-bearing human gate):** real `v0.2.2` published to public `bklim5/devtools-releases`; the **DST-02 updater round-trip** confirmed on Apple Silicon (older install → detect → minisign verify against the pinned pubkey → relaunch), Intel covered-by-construction (byte-identical signature under both keys). Two real bugs found+fixed during the live run (`main().catch()` false exit-1; updater install banner "8000%" → pure `downloadProgress.ts` reducer, TDD). **All 12 REL requirements (REL-01..12) complete.** Full gate green (vitest 503/503, tsc+eslint clean); decoder + its 19 tests byte-untouched; zero new runtime deps. Next: archive v1.2 (`/gsd-complete-milestone`); the CI track remains parked in backlog 999.2.
