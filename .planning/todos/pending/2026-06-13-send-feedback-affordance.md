...
1. **Capability scope** — 20-01 scoped the opener capability to `https://*` only. Add a tight `mailto:` scope entry to `src-tauri/capabilities/default.json` (the `opener:allow-open-url` scope), e.g. `mailto:feedback@tinkerdev.io*` — keep it narrow (only the support address, no arbitrary-scheme opening). Verify the tauri-plugin-opener scope format accepts non-https schemes.
