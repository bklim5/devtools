...
- [x] **SET-07**: The **Appearance** pane lets the user choose theme (light/dark/system) and accent, persisted via the existing prefs seam and applied live (absorbs backlog 999.3). _(Plan 23-01 landed the foundation — widened ThemeName/coerceTheme, accent scale + light tokens + AA contrast spec, pure apply helpers; the pane UI + live apply land in Plans 23-02..04, so the requirement is not yet fully met.)_
