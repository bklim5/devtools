...
- [ ] **REL-07**: The release is published to the public `bklim5/devtools-releases` repo (`gh release create --repo`), uploading the DMG + `.app.tar.gz` + `latest.json` — driver wired + dry-run-proven in Phase 11 Plan 02 (`gh release create` assets-first / `gh release upload latest.json` last, every call `--repo bklim5/devtools-releases`); the **live publish** is Plan 11-03's human gate
