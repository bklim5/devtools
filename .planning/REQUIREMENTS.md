...
- [ ] **FMT-01**: User can paste JSON and see it validated instantly; on invalid input the error is shown as line:col + message
- [ ] **FMT-02**: User can prettify JSON with a selectable indent (2 spaces / 4 spaces / tab)
- [ ] **FMT-03**: User can minify (compress) JSON to a single line
- [ ] **FMT-04**: User can sort JSON object keys recursively (array order preserved)
...
- [ ] **FMT-07**: User can minify (compress) XML by stripping insignificant inter-element whitespace
- [ ] **FMT-08**: User can copy the formatted output via a visible, keyboard-focusable control (no hover-only)
