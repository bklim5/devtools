...
const config = { from: "TinkerDev Licenses <licenses@tinkerdev.io>" };
...
describe("sendKeyEmail (D-64/D-66)", () => {
  it("calls emails.send once with the from address, buyer to, subject, and a plain-text body", async () => {
    ...
    expect(payload.from).toBe("TinkerDev Licenses <licenses@tinkerdev.io>");
    expect(payload.to).toEqual(["buyer@example.com"]);
    expect(payload.subject).toBe("Your TinkerDev license key");
    expect(typeof payload.text).toBe("string");
  });

  it("includes the key, the 3 activation steps, and the releases download link in the text", async () => {
    const { client, send } = makeSender();
    ...
    expect(text).toContain("github.com/bklim5/devtools-releases/releases/latest");
  });
